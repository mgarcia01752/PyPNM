...
| Path | Purpose |
|------|---------|
| `compose/docker-compose.yml` | Docker Compose stack that pulls `ghcr.io/PyPNMApps/pypnm:<tag>` and wires up the required volumes. |
...
1. **Optional – Grab the release bundle**
   - Download `pypnm-deploy-<version>.tar.gz` from the [latest release](https://github.com/PyPNMApps/PyPNM/releases).
