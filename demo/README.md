...
```bash
./tools/pnm-demo-sanitizer.py --copy-data --demo-root demo
```
...
```bash
./tools/pnm-demo-sanitizer.py --clean-demo \
                              --demo-root demo
```
...
```bash
./tools/pnm-demo-sanitizer.py   --force-new-mac               \
                                --new-mac aa:bb:cc:dd:ee:ff   \
                                --demo-root demo              \
                                --verbose
```
...
The sanitizer drives `tools/pnm-mac-updater.py` for you, but you can also run it directly to inspect or adjust PNM
files in place:

```bash
# Show MAC and type information for all PNM files in the demo dataset
./tools/pnm-mac-updater.py --all-files demo/.demo/pnm/ --show-info

# Rewrite MAC addresses in a single PNM file
./tools/pnm-mac-updater.py    --mac-address aa:bb:cc:dd:ee:ff   \
