...
class ConstellationDisplayRouter:
    """
    FastAPI router for Downstream OFDM Constellation Display using a single /getCapture endpoint.

    The endpoint triggers a capture on the cable modem, processes the results into an Analysis,
    and returns either JSON (analysis + primitive payload) or a downloadable archive containing
    plots and CSVs, mirroring the RxMerRouter FastAPI design.
    """
    ...
    def __routes(self) -> None:
        ...
        async def get_capture(request: PnmConstellationDisplayAnalysisRequest):
            ...
            Produces either:
            - JSON: `PnmAnalysisResponse` with analysis results merged with the primitive payload.
            - ARCHIVE: a downloadable ZIP with plots/CSVs generated by `ConstellationDisplayReport`.
            """
