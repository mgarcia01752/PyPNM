...
class _DummySettings:
    """
    Lightweight Settings Stub For JsonTransactionDb Tests.

    This stub provides the minimal attributes consumed by JsonTransactionDb:
    * json_db  – transaction DB filename
    * json_dir – base directory for JSON payloads / DB files
    * transaction_db_root, transaction_db_path, data_root, pnm_data_root – optional
      roots that may be inspected by _default_base_path().
    """

    def __init__(self, json_db: str, json_dir: str) -> None:
        self.json_db             = json_db
        self.json_dir            = json_dir
        self.transaction_db_root = None
        self.transaction_db_path = None
        self.data_root           = None
        self.pnm_data_root       = None
...
def _make_db(tmp_path: Path) -> JsonTransactionDb:
    """
    Helper To Construct JsonTransactionDb With A Temporary Filesystem Root.

    The DB file is named 'transactions.json' and the json_dir is the same as
    tmp_path / 'json' to keep payloads and DB co-located for tests.
    """
    json_dir: Path = tmp_path / "json"
    json_dir.mkdir(parents=True, exist_ok=True)

    settings_stub = _DummySettings(
        json_db="transactions.json",
        json_dir=str(json_dir),
    )

    db = JsonTransactionDb(settings=cast(Any, settings_stub))
    return db


def test_write_json_creates_payload_and_updates_db(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    fixed_time: int = 1_700_000_010
    ...
    updated_db: JsonTransactionDbModel = db.write_json(payload)
    ...
    db_path: Path      = tmp_path / "json" / "transactions.json"
    ...
    with open(payload_path, "rb") as handle:
        for chunk in iter(lambda: handle.read(8192), b""):
            digest.update(chunk)
    digest.update(str(fixed_time).encode("utf-8"))
    expected_hash: str = digest.hexdigest()
...
def test_read_json_roundtrip(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    updated_db = db.write_json(payload)
...
def test_read_json_missing_transaction_returns_empty(tmp_path: Path) -> None:
    db = _make_db(tmp_path)
...
def test_write_json_raises_on_non_serializable_data(tmp_path: Path) -> None:
    db = _make_db(tmp_path)
