...
\[
\begin{aligned}
i_{\min}[k] &= \min_{0 \le m < M} I_{m,k}, \\
i_{\text{avg}}[k] &= \frac{1}{M} \sum_{m=0}^{M-1} I_{m,k}, \\
...
These statistics characterise how the in-phase and quadrature components of the channel vary across
...
consistent with other phase-sensitive analyses (e.g., group delay or echo detection).
...
- Real part:
  \[
  r_{\min}[k],\quad r_{\text{avg}}[k],\quad r_{\max}[k].
  \]

- Imaginary part:
  \[
  i_{\min}[k],\quad i_{\text{avg}}[k],\quad i_{\max}[k].
  \]

- Magnitude:
  \[
  a_{\min}[k],\quad a_{\text{avg}}[k],\quad a_{\max}[k].
  \]
