...
- Script: [`tools/pnm-mac-updater.py`](http://github.com/mgarcia01752/PyPNM/blob/main/tools/pnm-mac-updater.py)
...
```bash
tools/pnm-mac-updater.py [OPTIONS]
```
...
```bash
tools/pnm-mac-updater.py   --show-info   --all-files .data/pnm
```
...
```bash
tools/pnm-mac-updater.py --list-types
```
...
```bash
tools/pnm-mac-updater.py   --mac-address aa:bb:cc:dd:ee:ff   --all-files .data/pnm   --only-types RECEIVE_MODULATION_ERROR_RATIO SPECTRUM_ANALYSIS   --dry-run
```
...
```bash
tools/pnm-mac-updater.py   --mac-address aa:bb:cc:dd:ee:ff   --all-files .data/pnm   --only-types RECEIVE_MODULATION_ERROR_RATIO SPECTRUM_ANALYSIS
```
...
```bash
tools/pnm-mac-updater.py   --mac-address de:ad:be:ef:00:bb   --all-files .data/pnm
```
...
```bash
tools/pnm-mac-updater.py   --mac-address aa:bb:cc:dd:ee:ff   --file .data/pnm/ds_ofdm_rxmer_per_subcar_0050f1120360_193_1764694042.bin
```
